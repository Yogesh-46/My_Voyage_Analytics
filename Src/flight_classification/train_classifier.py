...
df_clean["first_name"] = df_clean["name"].str.split().str[0]

print("Sample First Names:")
print(df_clean["first_name"].head())
...
# Encode the first name into numerical values
name_encoder = LabelEncoder()
df_clean['first_name_encoded'] = name_encoder.fit_transform(df_clean['first_name'])

# Split into feature matrix (X) and target vector (y)
X = df_clean[['age', 'company_encoded','first_name_encoded']]
...
classifier = RandomForestClassifier(n_estimators=100, max_depth=6,min_samples_split=2,min_samples_leaf=1,random_state=42)
...
joblib.dump(name_encoder, os.path.join(MODEL_DIR, "name_encoder.pkl"))

print("Classification model and encoders saved successfully.")